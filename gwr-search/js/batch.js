/* ── Batch CSV mode ────────────────────────────────────────────────────────
   Upload a CSV → look up each EGID against the GWR layer → the FOUND buildings
   ...
   Coded GWR fields are written twice: a readable label (`OUT_KATEGORIE`) and
   the raw integer (`OUT_KATEGORIE_CODE`). */

import { getLang, t } from "./i18n.js";
import { findByEgid, lv95ToWgs84, esc } from "./gwr-api.js";
...
const DEMO_URL = "examples/gwr-beispiel.csv";
const EGID_RE = /^\d+$/;       // EGID is a positive integer

// Fixed OUT_ column set + order — used in the downloads. Coded fields carry
// both the resolved label and the raw `*_CODE`.
const OUT_COLUMNS = [
  "OUT_RESULT", "OUT_MESSAGE", "OUT_EGID", "OUT_EGRID", "OUT_NAME", "OUT_GEB_NR",
  "OUT_ADRESSE", "OUT_PLZ", "OUT_ORT", "OUT_GEMEINDE", "OUT_BFS_NR", "OUT_KANTON", "OUT_PARZELLE",
  "OUT_KATEGORIE", "OUT_KATEGORIE_CODE", "OUT_KLASSE", "OUT_KLASSE_CODE",
  "OUT_STATUS", "OUT_STATUS_CODE", "OUT_BAUJAHR", "OUT_BAUPERIODE",
  "OUT_FLAECHE_M2", "OUT_GESCHOSSE", "OUT_WOHNUNGEN",
  "OUT_HEIZUNG", "OUT_HEIZENERGIE", "OUT_WARMWASSER", "OUT_WWENERGIE",
  "OUT_KOORD_E", "OUT_KOORD_N", "OUT_LAT", "OUT_LON", "OUT_GWR_URL",
];

// ── State ──
let parsed = null;        // { headers, rows, delimiter }
let fileName = "";
let egidCol = "";         // chosen input column holding the EGID
let results = [];         // full IN_/OUT_ rows (+ hidden _geometry/_feature) for download
let columns = [];         // [IN_*, ...OUT_COLUMNS]
let viewResults = [];     // found buildings { featureId, attributes, geometry } for the shared view
let lastSummary = "";
...
async function onDemoRun(e) {
  ...
  try {
    ...
    fileName = "gwr-beispiel.csv";
    parsed = parseCSV(text);
    showMapping();
    startProcessing(); // EGID column auto-detected in showMapping
  } catch {
...
function showMapping() {
  ...
  // Populate column dropdown; auto-select a column literally named "egid".
  const auto = parsed.headers.find((h) => /^egid$/i.test(h.trim())) || "";
...
async function runBatch() {
  await loadCodes(); // ensure coded fields resolve to labels in the output
  ...
  // De-dupe the shared table view by featureId (a CSV may list the same EGID
...
async function processOne(inputRow, headers, lang, signal, cache) {
  const rawEgid = (inputRow[egridCol] || "").trim();

  if (!rawEgid) return makeRow(inputRow, headers, { OUT_RESULT: "error", OUT_MESSAGE: t("batch.msg.empty_egid") });
  if (!EGID_RE.test(rawEgid)) {
    return makeRow(inputRow, headers, { OUT_RESULT: "error", OUT_MESSAGE: t("batch.msg.invalid_egid") });
  }

  // Cache the in-flight promise (not the resolved value) so concurrent workers
  // hitting the same EGID share one request rather than racing past cache.has.
  // Evict failed lookups so a later duplicate of the same EGID can retry.
  const key = rawEgid;
  if (!cache.has(key)) {
    cache.set(key, findByEgid(rawEgid, { signal }).catch((err) => {
      cache.delete(key);
      throw err;
    }));
  }
  ...
  const en = feature.geometry; // [E, N] in LV95, or null
  const row = makeRow(inputRow, headers, mapAttributes(feature.attributes || {}, en, lang));
  row._feature = { featureId: feature.featureId, attributes: feature.attributes, geometry: en };
  if (en) row._geometry = { type: "Point", coordinates: lv95ToWgs84(en) };
  return row;
}

/** Map GWR attributes → OUT_ fields. `en` is the LV95 [E, N] point (or null). */
function mapAttributes(a, en, lang) {
  const wgs = en ? lv95ToWgs84(en) : null; // [lng, lat]
  return {
    OUT_RESULT: "found",
    OUT_MESSAGE: "",
    OUT_EGID: a.egid ?? "",
    ...
    OUT_NAME: a.gbez || "",
    OUT_GEB_NR: a.gebnr ?? "",
    OUT_ADRESSE: a.strname_deinr || "",
    OUT_PLZ: a.dplz4 ?? "",
    OUT_ORT: a.dplzname || "",
    OUT_GEMEINDE: a.ggdename || "",
    OUT_BFS_NR: a.ggdenr ?? "",
    OUT_KANTON: a.gdekt || "",
    OUT_PARZELLE: a.lparz || "",
    OUT_KATEGORIE: resolveCode("gkat", a.gkat, lang),
    OUT_KATEGORIE_CODE: a.gkat ?? "",
    OUT_KLASSE: resolveCode("gklas", a.gklas, lang),
    OUT_KLASSE_CODE: a.gklas ?? "",
    OUT_STATUS: resolveCode("gstat", a.gstat, lang),
    OUT_STATUS_CODE: a.gstat ?? "",
    OUT_BAUJAHR: a.gbauj ?? "",
    OUT_BAUPERIODE: resolveCode("gbaup", a.gbaup, lang),
    OUT_FLAECHE_M2: a.garea ?? "",
    OUT_GESCHOSSE: a.gastw ?? "",
    OUT_WOHNUNGEN: a.ganzwhg ?? "",
    OUT_HEIZUNG: resolveCode("gwaerzh1", a.gwaerzh1, lang),
    OUT_HEIZENERGIE: resolveCode("genh1", a.genh1, lang),
    OUT_WARMWASSER: resolveCode("gwaerzw1", a.gwaerzw1, lang),
    OUT_WWENERGIE: resolveCode("genw1", a.genw1, lang),
    OUT_KOORD_E: a.gkode ?? "",
    OUT_KOORD_N: a.gkodn ?? "",
    OUT_LAT: wgs ? wgs[1] : "",
    OUT_LON: wgs ? wgs[0] : "",
    OUT_GWR_URL: a.egid != null ? `https://www.housing-stat.ch/de/query/egid.html?egid=${a.egid}` : "",
  };
}
...
function exportName(ext) {
  const base = fileName.replace(/\.[^.]+$/, "") || "gwr";
  return `${base}_gwr.${ext}`;
}
